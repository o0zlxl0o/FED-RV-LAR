...
// 로딩구역 만들기 //////////////////defer 안쓰는 방법
// 대상 : window (최상위 js 객체)
// 이벤트 등록 메서드 :
// addEventListener(이벤트, 함수)
// -> 이벤트와 함수를 연결시는 메서드
// -> 이벤트 : js에 등록된 공식 이벤트명을 사용
// 예) click, scroll, mouseenter, keypress,...
// 함수는 외부함수 또는 직접 익명함수를 쓴다!
...
// 모두 로딩될 때까지 기다린다
// 2) DOMContentLoaded : 태그만 다 그리면 실행
...
function loadFn() {
  ...
  // 0. 요구사항 분석 :
  // 1) tv버튼 클릭시 기능을 수행함
  // 2) 확대/축소 버튼 클릭 : 확대/축소함
  // 3) 채널 버튼 클릭: 유튜브 동영상 변경
  // - 확대/축소 시 트랜지션 적용하기
  ...
  function controlTV() {
    ...
    // 값이 1.4이면 3, 3이면 1.4로 변경하기
    scaleValue =
      scaleValue === "1.4" ? "3" : "1.4";
    ...
    // 2) getComputedStyle(대상요소).속성명
    // -> 2번 방법은 실제로 화면에 셋팅된 CSS값을 읽어옴
    let scale = getComputedStyle(tv).scale;
    ...
    switch (className) {
      ...
      case "ch1":
        mv.src =
          "https://www.youtube.com/embed/SyapN6xDnT8?autoplay=1";
        breack;
      case "ch2":
        mv.src =
          "https://www.youtube.com/embed/rGKg7YFyt14?autoplay=1";
        breack;
      case "ch3":
        mv.src =
