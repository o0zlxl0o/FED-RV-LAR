...
function 김비서나와라() {
  ...
  // 3-2. 글자색 변경하기
  나변수.style.color = "white";

  // 3-3. 글자크기 변경하기
  나변수.style.fontSize = "40px";

  // 3-4. 글자그림자(x축, y축, 블러, 색)
  나변수.style.textShadow = "2px 2px 5px #000000";
  ...
  // JS에서 body는 특별히 취급하여
  // document.body로 취급한다.
  // document(돔:DOM) 도큐먼트 오브젝트 모델 문서★★★★★
  // document(돔:DOM) -> HTML, XML 문서의 프로그래밍(절차, 순서) interface 이다.

  var 나바디 = document.body;
  나바디.style.backgroundColor = "#000000";
  //   나바디.style.backgroundColor = 'linear-gradient(70deg, pink, yellow, lightblue)';
...
function 맘대로해라(헐, 헉스) {
  ...
  //www.banapresso.com/m/menu/JUICE%20&%20DRINK

  // 2. 대상선정 : .박스야 .원이야
  // 해당요소의 순번은 헐 변수에 들어있음!
  https: var 아파트 =
    document.querySelectorAll(".박스야 .원이야")[
      헐
    ];
  // -> 아래와 같이 써도 같은것임!
  ...
  /************************************************************* 
    ...
    1. querySelector(CSS 선택자) - 하나만 선택
    2. querySelectorAll(CSS 선택자) - 여러개 선택
    -> 2번 방식은 HTMLCollection을 반환한다!
    -> 따라서 lengthdhk item(순번)/[순번]을 사용한다!
  ...
  아파트.innerHTML = `<iframe id="my-apt"
    src="https://www.youtube.com/embed/8Ebqe2Dbzls?
    autoplay=1"
 ...
 *******************************************************/
 ...
 function 사각사각(){
    ...
    // 3. 변경내용 : top, 트랜지션, white height값 변경
    나야나.style.top = "0";
    ...
    나야나.style.textShadow = "5px 5px 3px #333333";
    나야나.style.innerHeight = window.innerHeight + "px";
