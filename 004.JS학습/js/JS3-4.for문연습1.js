...
// 이름이 있는 함수는 ;(세미콜론)붙이지 않는다.
function insertMini() {
  // this -> 호출한 요소 자신!
  // 1. 호출한 요소의 특정 개수속성 읽어오기 : data-num
  ...
  // [ 속성 값 셋팅법 2가지 ]
  // 1) 선택요소.속성명 = 값
  // 예 documentation.querySelector('img').src = "url";
  // 2) 선택요소.setAttribute("속성명", "값");
  // 예 documentation.querySelector('img)
  //    .setAttribute("속성명", "값");

  // 2. 함수 호출확인
  console.log("미니언즈 넣어봐봐~!!", num);

  // 3. 변경 대상 요소(miniSpace)에 이미지 넣기
  // += 대입연산자 : 기존값에 새로운 연산자를 더함
  for(let i = 0; i < num; i++) {
    miniSpace.innerHTML += `
